"""Parse and run the investigation queries in sql/investigation.sql over a panel.

Kept as a small module so the command-line script and the tests share the same code. The
queries are named with `-- name:` markers, each carrying a `--` description.
"""
...
def run(csv_path: str | Path, sql_text: str | None = None,
    ...
    """Run every query against the panel CSV; return {name: (description, result)}."""
    if sql_text is None:
